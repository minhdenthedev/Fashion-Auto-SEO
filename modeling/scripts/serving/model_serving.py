...
@app.post("/generate_vi")
async def generate_caption_vi(file: UploadFile = File(...), max_tokens: int = 100):
    try:
        # Read file once
        contents = await file.read()
        ...
        with torch.no_grad():
            outputs = model.generate(
                **inputs, 
                max_length=max_tokens,
                min_length=10,
                num_beams=5,  # higher beam yield better result but more computation
